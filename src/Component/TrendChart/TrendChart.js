...
const TrendChart = ({ value_Application, value_Year }) => {
  ...
  let arrMonth = []; //for storing max value of particular module

  datas.forEach((e) => {
    if (e.Application === value_Application && e.Year === value_Year) {
     
        /**storing different modules */
          ...
          // ############
/**storing different month */
          const present1 = arrMonth.some((d) => {
            return d === e.Month;
          });
          if (present1 === false) {
  ...
  for (let i = 0; i < arrModule.length; i++) {
    arrMonthCount[i] = [];
    for (let j = 0; j < arrMonth.length; j++) {
      arrMonthCount[i][j] = [];
    }
  }

  arrModule.forEach((module, i) => {
    arrMonth.forEach((Month, j) => {
      let maxx = 0;
      currentMonth = Month;
      datas.forEach((e) => {
        if (e.Application === value_Application && e.Month === Month) {
          if (e.Module === module) {
            if (currentMonth === e.Month) {
              if (e.Count > maxx) {
                maxx = e.Count;
                console.log(module,"-", Month);
              }
            }
          }
        }
      ...
      for (let k = i; k <= i; k++) {
        for (let m = j; m <= j; m++) {
          arrMonthCount[k][m] = maxx;
        }
      }
    });
  });
//   console.log(arrDate);
  console.log(arrMonthCount);
  ...
  var palette = [
    ...
    ["#E65540", "#F8ECC2", "#65A8A6", "#79896D", "#F5A2A2"],
